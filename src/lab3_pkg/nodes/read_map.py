...
class MapDisplay(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            OccupancyGrid, '/map', self.set_map, qos_profile=latching_qos)
    ...
    def set_map(self, occupancy_grid):
        ...
        self.mapimg = 100 - \
            np.array(occupancy_grid.data).reshape((height, width))
    ...
    def real_pose_cb(self, pose):
        ...
        robot_pose_pix_x = int(
            (self.robot_pose[0] - self.origin[0]) / self.map_resolution)
        robot_pose_pix_y = - \
            int((self.robot_pose[1] - self.origin[1]) /
                self.map_resolution) + self.mapimg.shape[0]
        robot_pose_pix = [robot_pose_pix_x,
                          robot_pose_pix_y, self.robot_pose[2]]
        mapimg_tmp = self.mapimg.copy()
        robot_radio = int((0.355/2.0) / self.map_resolution)
        cv2.circle(mapimg_tmp, tuple(
            robot_pose_pix[:2]), robot_radio, (0, 0, 255), -1)
